import { useState, useEffect } from "react";
import { LogOut, Menu, X } from "lucide-react"; // or any icon set you prefer
import { useNavigate } from "react-router-dom";

const Sidebar = ({ tabs }) => {
  const [isMobile, setIsMobile] = useState(window.innerWidth < 900);
  const [isOpen, setIsOpen] = useState(window.innerWidth >= 900);
  const navigate = useNavigate();

  // Handle resizing behavior
  useEffect(() => {
    const handleResize = () => {
      const mobile = window.innerWidth < 900;
      setIsMobile(mobile);
      setIsOpen(!mobile);
    };

    window.addEventListener("resize", handleResize);
    return () => window.removeEventListener("resize", handleResize);
  }, []);

  const toggleSidebar = () => setIsOpen((prev) => !prev);

  const handleLogout = () => {
    // Add logout logic (Firebase + Redux)
    console.log("Logging out...");
    navigate("/");
  };

  return (
    <>
      {/* Hamburger button (mobile only) */}
      {isMobile && !isOpen && (
        <button
          onClick={toggleSidebar}
          className="fixed top-3 left-3 z-50  text-white "
        >
          <Menu size={20} />
        </button>
      )}

      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full z-40 bg-black text-white shadow-md transition-all duration-300 ease-in-out
          ${isOpen ? "w-[250px] lg:w-[300px]" : "w-[48px]"} px-2 lg:px-4 py-6`}
      >
        {/* Close button (mobile only) */}
        {isMobile && isOpen && (
          <button onClick={toggleSidebar} className="absolute top-4 right-4">
            <X size={20} />
          </button>
        )}

        {/* Header */}
        {isOpen && (
          <div className="my-4 flex flex-col gap-2">
            <h1 className=" font-light whitespace-nowrap">
              {isOpen ? "Vistora" : ""}
            </h1>
            <div
              className={`${
                isMobile ? (isOpen ? "w-full" : "w-0") : ""
              } w-full h-[1px] bg-neutral-400`}
            ></div>
            <div className="flex justify-start gap-2 items-center">
              <div className="w-8 h-8 rounded-full bg-neutral-200"></div>
              <h2 className="text-[0.9rem]">Company Name</h2>
            </div>
          </div>
        )}

        {/* Tab list */}
        <nav className="flex flex-col gap-2 my-10">
          {tabs &&
            tabs.map((tab, idx) => (
              <div
                className={`w-full p-2 rounded-xl hover:bg-neutral-800 cursor-pointer text-sm flex gap-4 items-center ${
                  !isOpen && "justify-center"
                }`}
              >
                <span className="">{tab.icon}</span>
                <span
                  className={`${isOpen ? "flex" : "hidden"} text-[0.85rem]`}
                >
                  {tab.label}
                </span>
              </div>
            ))}
        </nav>

        {/* Fixed Logout Button */}
        <div className="absolute bottom-6 left-4 right-4">
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 text-sm text-white hover:text-red-400"
          >
            <LogOut size={16} />
            {isOpen && "Log Out"}
          </button>
        </div>
      </aside>
    </>
  );
};

export default Sidebar;
