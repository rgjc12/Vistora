...
const FAQItem = ({ question, answer, isOpen, onClick, bulletPoints = [], index }) => {
  ...
  return (
    <div
      className={`border border-red-200/30 rounded-lg overflow-hidden shadow-lg hover:shadow-xl 
        transition-all duration-500 backdrop-blur-sm transform ease-out 
      ...
      >
        ...
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center 
            bg-red-100 group-hover:bg-red-200 transition-all duration-300 
            ${isOpen ? "bg-red-200" : ""}`}
        >
          <svg
            className={`w-5 h-5 text-red-800 transform transition-transform duration-500 ease-in-out 
              ${isOpen ? "rotate-180" : ""}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            ...
            <ul className="space-y-2 text-gray-700 font-poppins">
              {bulletPoints.map((point, i) => (
                <li key={i} className="flex items-start">
                  <span className="inline-block w-2 h-2 rounded-full bg-red-500 mt-2 mr-2" />
                  <span>{point}</span>
                </li>
              ))}
            </ul>
