...
const Tasks = () => {
  ...
  const { recentTasks, activeProjects, loading, error, filters } = useSelector(state => state.tasks);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <select 
          ...
          onChange={(e) => dispatch(setFilter({ filterType: 'priority', value: e.target.value }))}
        >
        ...
        <select 
          className="px-3 py-2 border border-gray-300 rounded-md"
          value={filters.date}
          onChange={(e) => dispatch(setFilter({ filterType: 'date', value: e.target.value }))}
        >
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`px-2 py-1 text-xs rounded ${
                  task.status === 'Urgent' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
                  {task.status}
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                <div 
                  className={`h-2 rounded-full ${task.status === 'Urgent' ? 'bg-red-600' : 'bg-yellow-600'}`}
